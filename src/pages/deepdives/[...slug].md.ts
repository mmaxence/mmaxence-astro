...
export async function getStaticPaths() {
  const entries = await getCollection('deepdives');
  return entries
...
function buildRawMarkdown(entry: CollectionEntry<'deepdives'>): string {
  const canonical = `${SITE_URL}/deepdives/${entry.slug}/`;
...
export const GET: APIRoute = ({ props }) => {
  const entry = props.entry as CollectionEntry<'deepdives'>;
